...
impl ActionEntry {
    ...
    pub fn boundary(&self, panel: Option<&PanelContext>) -> CaseBoundary {
...
mod tests {
    ...
    fn suppression_reads_the_effective_tier_not_the_agents_declaration() {
        ...
        assert_eq!(item.boundary(ctx.as_ref()).effective, governance::RiskTier::High);
        assert!(item.boundary(ctx.as_ref()).is_member_visible());
    ...
    fn panels_are_addressed_by_author_and_d_tag_so_one_agent_cannot_clobber_another() {
        ...
        assert_eq!(
            item.boundary(Some(&ctx)).effective,
