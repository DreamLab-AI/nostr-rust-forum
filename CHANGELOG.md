...
- **Calibration is relay-authoritative on the client.** Selection is HMAC'd
  under a relay-held secret (an agent chooses its own `d` tag, so unkeyed
  selection was grindable), so the client reads
