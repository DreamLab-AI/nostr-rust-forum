...
| KF-10 | MEDIUM | `crates/nostr-bbs-core/src/governance.rs:343, :365, :596` | The seeded-probe digest rides the **raw signed 31402**. The relay keeps it out of `event_tags` and out of every REST projection of an undecided case, and the forum client never renders it before a 31403 exists — but the tag cannot be stripped from the envelope without invalidating the signature both clients verify strictly. An agent reading its own governance subscription can therefore tell a probe from a real request and answer it correctly, defeating the catch rate the probes exist to measure. | This is **ADR-2011's own `review_trigger`** and the reason its `implementation_status` is `partial`, not an oversight. Client-side blindness protects the *reviewer*, which is what the catch rate needs from the human side, and protects nothing against the *agent*. Closing it needs a scheme that survives raw-event inspection — committing to the probe out of band, or encrypting the marker to the relay — which is a protocol design, not a patch. | core maintainers (ADR-2011 review) |
